    ...
    <div class="p-6">
      ...
      <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-900">
          {{ isEdit ? 'Edit Machine Type' : 'Create New Machine Type' }}
        </h2>
        <p class="mt-1 text-sm text-gray-600">
          {{ isEdit ? 'Update machine type information' : 'Add a new machine type to the system' }}
        </p>
      </div>
      ...
      <form @submit.prevent="handleSubmit">
        <div class="max-h-[70vh] overflow-y-auto space-y-4 pr-2">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                :class="{ 'border-red-500': errors.type_id }"
              />
              <p v-if="errors.type_id" class="mt-1 text-xs text-red-600">{{ errors.type_id[0] }}</p>
            </div>
          ...
          <div>
            ...
            <select
              ...
              required
              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              :class="{ 'border-red-500': errors.machine_type }"
            >
            ...
            <p v-if="errors.machine_type" class="mt-1 text-xs text-red-600">{{ errors.machine_type[0] }}</p>
          </div>
        ...
        <div class="mt-6 flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
          ...
          >
            ...
            <span v-else>{{ isEdit ? 'Update Machine Type' : 'Create Machine Type' }}</span>
          </button>
        </div>
      </form>
    </div>
...
const isEdit = computed(() => !!props.machineType);
...
// Watch for machineType prop changes to populate form
watch(() => props.machineType, (newMachineType) => {
  if (newMachineType) {
    Object.keys(form).forEach(key => {
      form[key] = newMachineType[key] ?? form[key];
    });
  } else {
    resetForm();
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.errors) {
        ...
        alert(data.message || 'Failed to save machine type');
      }
    }
  } catch (error) {
    console.error('Error saving machine type:', error);
    alert('An error occurred while saving the machine type');
  } finally {
    loading.value = false;
  }
};
