    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div class="mb-6">
          ...
          <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
              <div class="flex items-center">
                ...
                <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Create Form</span>
              </div>
            </li>
          </ol>
        </nav>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create Production Form</h1>
            <p class="mt-1 text-sm text-gray-600">Add a new form to Section {{ sectionInfo.section_id }}</p>
          </div>
        </div>
      </div>
      ...
      <form v-else @submit.prevent="submitForm" class="space-y-6">
        ...
        <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl shadow-md p-6 border border-blue-100">
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 class="text-lg font-semibold text-blue-900">Creating form for:</h3>
...
const formData = ref({
  ...
  operator_ids: [],
  material_id: '' // Changed to single material
});
...
const fetchSectionInfo = async () => {
  ...
  try {
    const response = await axios.get(`/api/sections/${formData.value.section_id}`);
    if (response.data.success) {
      sectionInfo.value = response.data.data;
    }
  } catch (error) {
    console.error('Error fetching section:', error);
    alert('Failed to load section information');
    router.push('/orders');
  }
};
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/forms', formData.value);
    
    if (response.data.success) {
      alert('Form created successfully!');
      // Go back to section page
      router.push(`/sections/${formData.value.section_id}`);
    }
  } catch (error) {
