    ...
    <div class="p-6">
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600">
          {{ isEdit ? 'Update status information' : 'Add a new status to the system' }}
        </p>
      </div>
      ...
      <form @submit.prevent="handleSubmit">
        <div class="max-h-[70vh] overflow-y-auto space-y-4 pr-2">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              />
              <p v-if="errors.status_id" class="mt-1 text-xs text-red-600">{{ errors.status_id[0] }}</p>
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows="3"
              placeholder="Enter a description of this status..."
              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
        ...
        <div class="mt-6 flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
          ...
          >
            ...
            <span v-else>{{ isEdit ? 'Update Status' : 'Create Status' }}</span>
          </button>
        </div>
      </form>
    </div>
...
const isEdit = computed(() => !!props.statusItem);
...
watch(() => props.statusItem, (newStatus) => {
  if (newStatus) {
    Object.keys(form).forEach(key => {
      form[key] = newStatus[key] ?? form[key];
    });
  } else {
    resetForm();
  }
...
const handleSubmit = async () => {
  ...
  try {
    const url = isEdit.value 
      ? `/api/statuses/${props.statusItem.id}`
      : '/api/statuses';
